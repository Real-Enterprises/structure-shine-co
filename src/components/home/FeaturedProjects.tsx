...
export function FeaturedProjects({ projects }: Props) {
  ...
  const nextDesktop = () => setDesktopActive((a) => Math.min(desktopMax, a + 1));
          ...
          <div className="flex items-center gap-3 w-full">
            {projects.length > 1 && (
              <div className="flex gap-2">
                ...
                >
                  <PremiumIcon icon={premiumIcons.chevronLeft} className="w-5 h-5" strokeWidth={2} />
                </button>
                <button
                  onClick={nextMobile}
                  ...
                  className="w-11 h-11 rounded-full border border-border flex items-center justify-center text-foreground hover:bg-accent hover:border-accent hover:text-accent-foreground transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  <PremiumIcon icon={premiumIcons.chevronRight} className="w-5 h-5" strokeWidth={2} />
                </button>
              </div>
            )}
            <Link href="/portfolio" className="flex-1">
              <Button variant="outline" className="rounded-full w-full">
                View Gallery
                <PremiumIcon icon={premiumIcons.arrowUpRight} className="w-4 h-4 ml-2" strokeWidth={1.8} />
              </Button>
            </Link>
          </div>
        </div>
              ...
              {projects.map((project) => (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5">
                    ...
                    <div className="flex items-center gap-1.5 text-white/70 text-xs mb-3 opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition-all duration-300">
                      <PremiumIcon
                        icon={premiumIcons.mapPin}
                        className="w-3.5 h-3.5 text-accent flex-shrink-0"
                        strokeWidth={2}
